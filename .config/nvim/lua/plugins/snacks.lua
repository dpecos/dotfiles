...
-- Set a random ascii art header before initializing snacks
...
-- find
map("<leader>fc", function() Snacks.picker.files({ cwd = vim.fn.stdpath("config") }) end, "Snacks", "Find Config File")
...
map("<leader>sw", function() Snacks.picker.grep_word() end, "Snacks", "Visual selection or word", { mode = { "n", "x" } })
...
map("<leader>bo", function()
  ...
  vim.cmd("only")
end, "Buffers", "Delete Other Buffers")
...
map("]]", function() Snacks.words.jump(vim.v.count1) end, "Snacks", "Next Reference", { mode = { "n", "t" } })
map("[[", function() Snacks.words.jump(-vim.v.count1) end, "Snacks", "Prev Reference", { mode = { "n", "t" } })
